...
# No explicit closing tag for tool calls; use empty string
TOOL_CLOSE = ""
ARGUMENT_OPEN = "[ARGS]"
...
class Ministral3ToolParser(BaseToolParser):
    ...
    def parse(self, content: str) -> Tuple[Optional[Dict[str, Any]], str]:
        ...
        while True:
            ...
            # Find argument opening tag after the tool open tag
            start_argument = content.find(self.argument_open, start_tool + self._tool_open_len)
            if start_argument == -1:
            ...
            # Extract the tool name between opening and argument tags
            tool_name = content[start_tool + self._tool_open_len:start_argument].strip()
    ...
    def parse_stream(self, chunk: Optional[str] = None) -> Tuple[Optional[Dict[str, Any]], bool]:
        ...
        if self.ministral3_tool_state == Ministral3ToolState.FOUND_ARGUMENTS:
            ...
            if end_tool_index != -1:
                ...
                # Convert arguments dict to JSON string for consistency with other parsers
                res["arguments"] = json.dumps(json_output["arguments"]) if isinstance(json_output["arguments"], dict) else str(json_output["arguments"])
